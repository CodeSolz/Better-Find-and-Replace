...
class Scripts_Settings {
    ...
    public static function load_admin_settings_scripts( $page_id ){
        ...
        wp_enqueue_style  ( 'sweetalert', CS_RTAAFR_PLUGIN_ASSET_URI . 'plugins/sweetalert/dist/sweetalert.css', false ); 
        wp_enqueue_script ( 'sweetalert', CS_RTAAFR_PLUGIN_ASSET_URI . 'plugins/sweetalert/dist/sweetalert.min.js', false ); 

        if( isset($rtafr_menu['add_new_coin']) && $page_id == $rtafr_menu['add_new_coin'] ){
            wp_enqueue_style ( 'jquery-typehead', CS_RTAAFR_PLUGIN_ASSET_URI . 'plugins/jquery-typeahead/jquery.typeahead.min.css', false ); 
            wp_enqueue_script ( 'jquery-typehead-js', CS_RTAAFR_PLUGIN_ASSET_URI . 'plugins/jquery-typeahead/jquery.typeahead.min.js', false ); 
            wp_enqueue_style ( 'jquery-date-time-picker', CS_RTAAFR_PLUGIN_ASSET_URI . 'plugins/jquery-date-time-picker/jquery.datetimepicker.min.css', false ); 
            wp_enqueue_script ( 'jquery-date-time-picker', CS_RTAAFR_PLUGIN_ASSET_URI . 'plugins/jquery-date-time-picker/jquery.datetimepicker.full.min.js', false ); 
        }
        
        if( isset($rtafr_menu['product_page_options_settings']) && $page_id == $rtafr_menu['product_page_options_settings'] ){
            wp_enqueue_style ( 'select2', CS_RTAAFR_PLUGIN_ASSET_URI . 'plugins/select2/dist/css/select2.min.css', false ); 
            wp_enqueue_script ( 'select2', CS_RTAAFR_PLUGIN_ASSET_URI . 'plugins/select2/dist/js/select2.min.js', false ); 
        }
        
        wp_enqueue_media();
        wp_enqueue_script ( 'media-uploader', CS_RTAAFR_PLUGIN_ASSET_URI . 'js/wp_media_uploader.js', false ); 
        wp_enqueue_style  ( 'wapg', CS_RTAAFR_PLUGIN_ASSET_URI . 'css/style.css', false ); 
    }
    ...
    public static function load_admin_footer_script( $page_id ){
        ...
        if( $page_id == $rtafr_menu['add_new_coin'] ){
        ...
        if( $page_id == $rtafr_menu['add_new_coin'] ||
            $page_id == $rtafr_menu['default_settings'] ||
            $page_id == $rtafr_menu['register_automatic_order'] ||
            $page_id == $rtafr_menu['checkout_options_settings'] ||
            $page_id == $rtafr_menu['product_page_options_settings'] 
            ){
            self::form_submitter();
        }
        
        if( $page_id == $rtafr_menu['all_coins_list']
            ){
            self::show_more_less();
        }
            ...
            <script type="text/javascript">
                jQuery(document).ready(function( $ ){
                    $("form").submit(function(e){
                        ...
                        swal({ title: $this.find('#cs_field_swal_title').val(), text: 'Please wait a while...', timer: 200000, imageUrl: '<?php echo CS_RTAAFR_PLUGIN_ASSET_URI .'img/loading-timer.gif'; ?>', showConfirmButton: false, html :true });
                        $.ajax({
                            url: ajaxurl,
                            type: 'POST',
                            data: formData,
